...
- 전처리 단계(normalize / Otsu)가 W2 이후 deep learning과 어떻게 연결될지 — 본인의 예측
- 본 binary 데이터에서 Otsu가 보이지 않는 진가 — 어떤 상황(grayscale·노이즈·도메인 변화)에서 의미가 살아날지
- 4 도메인의 등방성 순위 + 본인의 판단 근거 (어떤 지표로 평가했는가)
- B1 Linear vs B2 Cubic의 차이가 도메인·k에 따라 어떻게 달라지는지 + 그 차이의 원인에 대한 본인 가설
- 본인이 측정한 motivation 곡선이 \"왜 deep learning이 필요한가\"를 어떻게 정량적으로 설명하는지
