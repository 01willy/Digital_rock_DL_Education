...
window.COURSE = {
  meta: {
    ...
    repo: 'Digital_rock_DL_Education',
  },
  ...
  weeks: [
    ...
    },
    {
      ...
      title:'다른 아키텍처 비교',
      en:'UNet vs SwinUNet vs 3D UNet',
      summary:'같은 sparse 보간 문제를 UNetG(2D CNN), SwinUNet(Transformer), 3D UNet으로 풀어 비교. 학습 시간·파라미터·정확도의 trade-off를 정량 분석. 1조는 모델 swap 실험, 2조는 결과 해석.',
      concepts:['SwinUNet','Window attention','3D UNet','파라미터·시간 trade-off','Pre-trained ckpt 활용'],
      nonDL:false,
      plan:['UNetG vs SwinUNet vs 3D UNet 아키텍처 비교','Pre-trained 3개 ckpt 로드 + inference','평가지표 다섯 가지 비교 표 작성','연구 결과 fig_supp_swinunet 해석'],
      newUtils:['SwinUNetMini','UNet3DMini','model_zoo','benchmark_models'],
      prep:['Pre-trained 3 ckpt 다운로드 (config 안내 따라)'],
    },
    {
      n:5, status:'upcoming', slug:'w5', available:false,
      title:'Tri-Axis Aggregation — 본 연구 핵심',
      en:'2.5-D Fusion (3 methods)',
      summary:'세 직교축(z·y·x) 보간 결과를 GT-free 방식으로 융합. tri_mean / tri_weuler_self / tri_consensus 세 방식을 직접 실행·비교. 등방성 가정을 4 도메인에서 정량 검증.',
      concepts:['Tri-axis','GT-free aggregation','tri_mean','Euler-weighted','등방성 검증'],
      nonDL:false,
      plan:['z·y·x 각 축 inference 직접 실행','3가지 aggregation 방법 비교 (mean/weuler_self/consensus)','4 도메인(BB/CG/Bentheimer/Parker)에서 평가','연구 결과 fig_aggregation_pareto 해석'],
      newUtils:['triaxis_inference','aggregate_mean','aggregate_weuler','aggregate_consensus'],
      prep:['W4 pre-trained ckpt 또는 W2 학습한 mini UNet'],
    },
    {
      n:6, status:'upcoming', slug:'w6', available:false,
      title:'Cross-domain Transfer + Carbonate',
      en:'Sandstone → Carbonate · LBM',
      summary:'Ketton·Estaillades 탄산염암(carbonate) 도메인에 Zero-shot vs Fine-tune 30분 비교. 인코딩 차이(0/1 invert) sanity check 학습. LBM permeability 결과로 최종 평가. 학생 발표로 마무리.',
      concepts:['Cross-domain','Zero-shot vs FT','Carbonate','LBM permeability','Encoding sanity'],
      nonDL:false,
      plan:['Ketton·Estaillades 데이터 불러오기 + 인코딩 sanity check','Zero-shot 평가 vs Fine-tune 30분 결과 비교','LBM permeability (사전 계산 결과 시각화)','학생 발표 + 종합 회고'],
      newUtils:['load_carbonate','encoding_check','fine_tune_short','lbm_results_viz'],
      prep:['전체 주차 결과 정리','발표 자료 5분 분량'],
    },
  ],
